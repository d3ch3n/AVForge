...
Esta primeira fase é exclusivamente a modelagem estruturada de equipamentos. Ainda não há agentes, IA, banco de dados, código de aplicação ou geração de desenhos. A versão 3 estrutural do schema está em `schemas/equipment.schema.json` e os equipamentos ficam em `equipment/<vendor>/`.
...
`communication_capabilities` modela protocolos de rede sem duplicar a porta física. Cada capability referencia interfaces por IDs e declara explicitamente seu modo de associação (`fixed`, `configurable`, `simultaneous`, `redundant` ou `segregated`). Capacidades numéricas ficam em `capacity`; `resource_pools` representam recursos compartilhados; `capability_modifiers` representam licenças, módulos e outras condições que alteram disponibilidade ou capacidade.
