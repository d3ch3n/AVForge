...
Esta primeira fase é exclusivamente a modelagem estruturada de equipamentos. Ainda não há agentes, IA, banco de dados, código de aplicação ou geração de desenhos. A versão 3.10 estrutural do schema está em `schemas/equipment.schema.json` e os equipamentos ficam em `equipment/<vendor>/`.
...
`modularity` classifica equipamentos como `standalone`, `chassis`, `module` ou `hybrid`. Chassis declaram slots individuais, cada slot pode declarar `accepts.module_types`, e módulos declaram `module_type` e `compatible_slot_types`. O slot declara o que aceita; o módulo declara onde pode entrar. Essas declarações são complementares e não são validadas semanticamente pelo schema. O catálogo descreve slots e possibilidades, mas não módulos instalados em uma unidade específica; essa composição será uma camada futura.
